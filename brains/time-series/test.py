...
normalize = layers.Normalization()(inputs)

rnn = layers.LSTM(units=rnn_width, return_sequences=True)(normalize)
...
fig.add_trace(go.Scatter(x=data['Datetime'], y=yhat.squeeze(), mode='lines', name='Prediction', line=dict(color='red')), row=1, col=1)
