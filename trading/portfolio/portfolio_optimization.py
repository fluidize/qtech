...
"""))
    ...
    def portfolio_performance(self, weights, risk_free_rate=0.0) -> Dict[str, float]:
        ...
        if len(downside_returns) > 0:
            downside_vol = np.sqrt(np.mean((downside_returns - risk_free_rate / 252) ** 2)) * np.sqrt(252)
        else:
            downside_vol = 0.0  # No downside risk observed

        # Sortino ratio
        sortino = (portfolio_return - risk_free_rate) / downside_vol if downside_vol > 0 else 0
    ...
    def optimize(self, objective='sharpe', risk_free_rate=0.0) -> np.ndarray:
        ...
        if objective == 'sharpe':
            def neg_sharpe(weights, risk_free_rate):
                return -self.portfolio_performance(weights, risk_free_rate)["sharpe"]
            ...
            def neg_sortino(weights, risk_free_rate):
                return -self.portfolio_performance(weights, risk_free_rate)["sortino"]
            ...
            def port_vol(weights, _):
                return self.portfolio_performance(weights, risk_free_rate)["portfolio_vol"]
    ...
    def summary(self, weights, risk_free_rate=0.0) -> Dict:
        ...
        return {
            'weights': weights,
            'annual_return': perf["portfolio_return"],
            'annual_volatility': perf["portfolio_vol"],
            'downside_volatility': perf["downside_vol"],
    ...
    def print_portfolio_summary(self, weights: np.ndarray, portfolio_type: str, risk_free_rate=0.0):
        ...
        metrics_table.add_row("Downside Volatility", f"{summary['downside_volatility']:.2%}")
        metrics_table.add_row("Sharpe Ratio", f"{summary['sharpe_ratio']:.2f}")
        metrics_table.add_row("Sortino Ratio", f"{summary['sortino_ratio']:.2f}")
        
        # Print tables
        self.console.print(weights_table)
        self.console.print(metrics_table)

    def plot_portfolio_weights(self, weights_sharpe, weights_vol, weights_sortino=None, save_path=None):
        if weights_sortino is not None:
    ...
    def generate_random_portfolios(self, num_portfolios=5000, risk_free_rate=0.0) -> Tuple[List[float], List[float], List[float]]:
        returns = []
        volatilities = []
        sharpe_ratios = []
        ...
        with Progress(
            ...
            for i in range(num_portfolios):
                ...
                returns.append(outputs["portfolio_return"])
                volatilities.append(outputs["portfolio_vol"])
                sharpe_ratios.append(outputs["sharpe"])
    ...
    def plot_efficient_frontier(self, weights_sharpe, weights_vol, risk_free_rate=0.0, num_portfolios=5000, save_path=None):
        ...
        # Create a standard 2D scatter plot of the efficient frontier
        plt.figure(figsize=(10, 6))
        plt.scatter(volatilities, returns, c=sharpe_ratios, cmap='viridis', alpha=0.5)
        plt.colorbar(label='Sharpe Ratio')
        
        max_sharpe_outputs = self.portfolio_performance(weights_sharpe, risk_free_rate)
        min_vol_outputs = self.portfolio_performance(weights_vol)
        
        max_sharpe_ret = max_sharpe_outputs["portfolio_return"]
        max_sharpe_vol = max_sharpe_outputs["portfolio_vol"]
        min_vol_ret = min_vol_outputs["portfolio_return"]
        min_vol_vol = min_vol_outputs["portfolio_vol"]
    ...
    def optimize_and_plot(self, risk_free_rate=0.0, num_portfolios=5000, optimize_sortino=False):
        ...
        self.plot_portfolio_weights(weights_sharpe, weights_vol, weights_sortino, save_path='optimal_portfolio_allocation.png')
        ...
        if optimize_sortino:
            return weights_sharpe, weights_vol, weights_sortino
        else:
...
if __name__ == "__main__":
    ...
    optimizer.optimize_and_plot(risk_free_rate=0.00, optimize_sortino=True)
