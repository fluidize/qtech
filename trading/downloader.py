...
class MultiDownloadManager(QThread):
    """Manager thread for handling multiple symbol-interval combination downloads"""
    progress_update = pyqtSignal(str, str)  # task_id, message
    download_complete = pyqtSignal(str, object, str)  # task_id, data, status message
    error_occurred = pyqtSignal(str, str)  # task_id, error message
    ...
    def __init__(self, symbols, days, intervals, age_days, data_source, max_workers=5):
        ...
        # Create combinations of symbols × intervals
        self.download_tasks = []
        ...
        self.total_count = len(self.download_tasks)
        self.mutex = QMutex()
        
    def run(self):
        """Run the multi-download process"""
        self.progress_update.emit("SYSTEM", f"Starting downloads for {self.total_count} symbol-interval combinations with {self.max_workers} workers")
        self.progress_update.emit("SYSTEM", f"Combinations: {len(self.symbols)} symbols × {len(self.intervals)} intervals = {self.total_count} tasks")
        
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # Submit all download tasks
            future_to_task = {}
            for task in self.download_tasks:
                future = executor.submit(self.download_single_combination, task)
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    self.download_complete.emit(task['task_id'], data, status)
    ...
    def download_single_combination(self, task):
        """Download data for a single symbol-interval combination (runs in thread pool)"""
        try:
            task_id = task['task_id']
            symbol = task['symbol']
            interval = task['interval']
            
            self.progress_update.emit(task_id, f"Starting download for {symbol} ({interval})")
            ...
            self.progress_update.emit(task_id, "Download completed successfully!")
            self.progress_update.emit(task_id, f"Data shape: {data.shape}")
            
            if not data.empty:
                self.progress_update.emit(task_id, f"Date range: {data['Datetime'].min()} to {data['Datetime'].max()}")
                return data, "Download completed successfully!"
            else:
                self.progress_update.emit(task_id, "Warning: No data was downloaded")
                return data, "No data downloaded"
                
        except Exception as e:
            error_msg = f"Error during download: {str(e)}"
            self.progress_update.emit(task_id, error_msg)
            raise e
...
class DataDownloaderApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.symbol_edit = QLineEdit("BTC-USDT, ETH-USDT, SOL-USDT")
        ...
        self.interval_edit = QLineEdit("30m, 1h, 4h")
...
• Max Workers: Number of parallel downloads (1-20, default: 5)
...
The app will create combinations of symbols × intervals and download them simultaneously.
Example: 3 symbols × 3 intervals = 9 total downloads in parallel.
...
Cache expires after 24 hours. Retry limit is set to 5 attempts.
    ...
    def update_task_progress(self, task_id, status):
        """Update the progress list for individual symbol-interval combinations"""
        ...
        if existing_item:
            existing_item.setText(f"{task_id}: {status}")
        else:
            new_item = QListWidgetItem(f"{task_id}: {status}")
    ...
    def start_download(self):
        ...
        try:
            ...
            if max_workers < 1 or max_workers > 20:
                QMessageBox.warning(self, "Error", "Max workers must be between 1 and 20")
                return
        ...
        self.progress_label.setText(f"Downloading {total_combinations} combinations ({len(symbols)} symbols × {len(intervals)} intervals)...")
    ...
    def download_complete(self, task_id, data, status_message):
        """Handle successful download completion for a single symbol-interval combination"""
        self.downloaded_data[task_id] = data
        self.update_task_progress(task_id, "✓ Completed")
        
    def download_error(self, task_id, error_message):
        """Handle download error for a single symbol-interval combination"""
        self.update_task_progress(task_id, f"✗ Error: {error_message}")
        
    def update_overall_progress(self, completed, total):
        """Update overall progress bar"""
        self.progress_bar.setValue(completed)
        self.progress_label.setText(f"Downloaded {completed}/{total} combinations...")
        ...
        if completed >= total:
            ...
            if failed > 0:
                ...
                QMessageBox.information(self, "Download Complete", 
                                      f"All {successful} symbol-interval combinations downloaded successfully!")
